  ...
  methods: {
    ...
    uploadChange(file) {
      ...
      if (this.type == 1) {
        ...
        that.$post("upload", uploadFile, 1).then(res => {
          if (res.code == 200) {
            that.isloading = false;
            that.videoId = res.data.id;
            that.videoUrl = res.data.path;
            this.$emit("uploadVideoId", res.data.id);
          }
        });
      }
      // 文档
      if (this.type == 2) {
        ...
        that.$post("upload", uploadFile, 1).then(res => {
          if (res.code == 200) {
            that.isloading = false;
            that.notesId = res.data.id;
            that.notesName.push(res.data);
            this.$emit("uploadNotesId", res.data.id);
          }
        });
      }
    },
      ...
      this.$refs.cropper.getCropBlob(data => {
        ...
        that.$post("upload", uploadFile, 1).then(res => {
          if (res.code == 200) {
            ...
            that.uploadImgUrl = res.data.path;
            this.$emit("uploadImgId", res.data.id);
          }
        });
      });
    },
