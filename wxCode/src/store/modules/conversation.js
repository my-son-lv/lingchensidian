...
const conversationModules = {
  ...
  getters: {
    ...
    totalUnreadCount: (state) => {
      const result = state.allConversation.reduce(
  ...
  mutations: {
    ...
    unshiftMessageList(state, messageList) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (typeof list[i].payload.data == 'string') {
          list[i].payload.data = JSON.parse(message.payload.data);
        } else {
          console.log("不需要转换");
        }
      }
    ...
    receiveMessage(state, messageList) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        let date = new Date(message.time * 1000);
        list[i].newtime = formatTime(date);
        if (list[i].payload.description == "推荐外教") {
          list[i].payload.data = JSON.parse(message.payload.data);
        }
      }
    ...
    removeMessage(state, message) {
      state.currentMessageList.splice(
        state.currentMessageList.findIndex((item) => item.ID === message.ID),
        1
      );
    },
    ...
    checkoutConversation(context, conversationID) {
      ...
      return wx.$app
        ...
        .then(({ data: { conversation } }) => {
          ...
          wx.navigateTo({
            url: `../chat/main?toAccount=${name}&type=${conversation.type}`,
          });
